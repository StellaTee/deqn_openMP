...
void ExplicitScheme::reset()
{
    ...
    // #pragma omp parallel for collapse(2) slower
...
void ExplicitScheme::diffuse(double dt)
{
    ...
    // #pragma omp parallel for collapse(2) slower
    #pragma omp parallel for schedule(static) shared(u0, u1, rx, ry, nx)
...
void ExplicitScheme::getTime()
    {
        std::cout << "\t\tDiffuse looptime taken: " << diffuseTimer  << std::endl;
        std::cout << "\t\tReset looptime taken: " << resetTimer  << std::endl;
        std::cout << "\t\tBoundary looptime taken: " << boundaryTimer << std::endl;
    }