...
double Mesh::getTotalTemperature()
{
    if(allocated) {
        ...
        // #pragma omp parallel for reduction(+:temperature) collapse(2) slower
        #pragma omp parallel for reduction(+:temperature) schedule(static)
...
void Mesh::getTime() 
    {
        std::cout << "\t\tTemperature looptime taken: " << temperatureTimer << std::endl;
    }